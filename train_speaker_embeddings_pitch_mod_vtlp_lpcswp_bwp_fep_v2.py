...
class SpeakerBrain(sb.core.Brain):
    ...
    def swp(self,wavt):
        ...
        for i in range(len(wava)):
            #print(len(y))
            p=18
            new_y = synthesis_slpcw_fep_bwp(wava[i], 16000, np.hamming(256), p, 0.5, 0.6, 0.5)
    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            wav_augm = []
            spk_id_augm = []
            for ai in range(len(wava)-self.batch_size):
                wav_augm.append(wava[self.batch_size+ai])
                spk_id_augm.append(spk_ida[self.batch_size+ai])
            #print(type(wav_augm))
            ...
            for ai in range(self.batch_size*3):
