...
class SpeakerBrain(sb.core.Brain):
    ...
    def swp(self,wavt):
        ...
        for i in range(len(wava)):
            #print(len(y))
            ## since the weightage of this augmentation is 3/8, we will pass only 3/8th fraction of batch to this augmentation to avoid un-necessary latency
            if i < int(len(wava)/3):
                p=18
                new_y = synthesis_slpcw_fep_bwp(wava[i], 16000, np.hamming(1024), p, 0.75, 0.6, 0.5)
                wav_swp.append(new_y)
            else:
                new_y = do_nothing(wava[i],16000, np.hamming(1024), p, 0.75, 0.6, 0.5)
                wav_swp.append(new_y)
    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            wava = wavs.detach().cpu().numpy()
            spk_ida1 =  torch.cat([spk_id] * self.n_augment, dim=0)
            spk_ida = spk_ida1.detach().cpu().numpy()
            #print(len(spk_ida),len(wava))
            wav_org = [] #list(wava[:self.batch_size])
            spk_id_org = []
            for ai in range(self.batch_size+int(len(wava)/3)):
                wav_org.append(wava[ai])
                spk_id_org.append(spk_ida[ai])
            #print(type(wava),type(wav_org),type(spk_id_org),type(spk_ida),type(wavs))
            wav_augm = []
            spk_id_augm = []
            for ai in range(len(wava)-self.batch_size-int(len(wava)/3)):
                wav_augm.append(wava[self.batch_size+int(len(wava)/3)+ai])
                spk_id_augm.append(spk_ida[self.batch_size+int(len(wava)/3)+ai])
            #print(type(wav_augm))
            temp = list(zip(wav_augm, spk_id_augm))
            random.shuffle(temp)
            res1, res2 = zip(*temp)
            # res1 and res2 come out as tuples, and so must be converted to lists.
            res1, res2 = list(res1), list(res2)
            #print(len(res1),type(res1[0]),res1[0],type(wav_org[0]))
            for ai in range(self.batch_size*3-int(len(wava)/3)):
                wav_org.append(res1[ai])
                spk_id_org.append(res2[ai])
            #wav_org.append(res1[:self.batch_size])
            #spk_id_org.append(res2[:self.batch_size])
            #print(wav_org, spk_id_org)
            #print(len(wav_org),len(spk_id_org))
            wavs = torch.FloatTensor(np.array(wav_org)).to(self.device)
            spk_id = torch.Tensor(np.array(spk_id_org)).to(self.device)
            #print(len(wavs),self.n_augment)
            lens = torch.cat([lens] * 4)#self.n_augment)
...
#########################################################################
        ...
        return outputs, lens, spk_id

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss using speaker-id as label.
        """
        predictions, lens, spk_id = predictions
        uttid = batch.id
        #spkid, _ = batch.spk_id_encoded

        # Concatenate labels (due to data augmentation)
        if stage == sb.Stage.TRAIN:
            spkid = spk_id #torch.cat([spkid] * self.n_augment, dim=0)
